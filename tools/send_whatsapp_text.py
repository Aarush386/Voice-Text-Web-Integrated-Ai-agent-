# send_whatsapp_text.py — text-only robust wrapper
import os
...
def send_whatsapp_text(to:str, body:str)->Dict[str,Any]:
    try:
        ...
        err=str(e).lower()
        if "invalid" in err or "phone" in err: return {"ok":False,"summary":"invalid_phone"}
        if "auth" in err: return {"ok":False,"summary":"twilio_auth_failed"}
